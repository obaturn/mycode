...
CORS(app, resources={r"/gospel": {"origins": "*"}})
...
def gospel_music(offset=0, limit=15):
...
def get_gospel_songs(data):
    offset = data.get('offset', 0)
    limit = data.get('limit', 15)

    tracks = gospel_music(offset, limit)
    if tracks:
        songs = []
        for track in tracks:
            artist_image_url = (
        ...
        return jsonify({"error": "No gospel songs found"}), 404
