...
CORS(app, resources={r"/HipHop": {"origins": "*"}})
...
def HipHop_music(offset=0, limit=15):
...
def get_HipHop_songs(data):
    offset = data.get('offset', 0)
    limit = data.get('limit', 15)

    tracks = HipHop_music(offset, limit)
    if tracks:
        songs = []
        for track in tracks:
            artist_image_url = (
        ...
        return jsonify({"error": "No HipHop songs found"}), 404
